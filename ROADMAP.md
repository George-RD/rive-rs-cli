...
**Next item after this PR merges: [#178 — prove the complex animated AuthoringSpec exit gate](https://github.com/George-RD/rive-rs-cli/issues/178).**
...
deepens the canonical SceneSpec compilation seam in PR #195. This PR (#196)
implements [#174](https://github.com/George-RD/rive-rs-cli/issues/174) by exposing
AuthoringSpec through the CLI without duplicating that shared seam. A fresh
implementation session should continue open PR #196 first; once it is merged,
claim #178 before selecting later work.
...
#176 one-pass motion compiler (complete in PR #192)
  ...
  -> #174 first-class AuthoringSpec CLI (implemented in PR #196)
...
| P1 | [Authoring delivery path](meta/todos/todo.authoring-delivery-path.md) | in progress; #176 complete in PR #192, #177 complete in PR #195, and #174 implemented in PR #196; #178 is next | one-pass compiler, one shared compile seam, first-class Authoring CLI, complex animated runtime proof |
...
- #176 completes the one-pass motion compiler architecture gate in PR #192.
- #177 completes one canonical `SceneSpec -> .riv` application seam in PR #195 so
  #174 can add AuthoringSpec input without copying the raw generate/build/encode
  flow.
- #174 exposes AuthoringSpec explicitly through the CLI in PR #196 while preserving
  raw SceneSpec `generate` as the expert path.
