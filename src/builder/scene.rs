...
pub fn build_scene(spec: &SceneSpec) -> Result<Vec<Box<dyn RiveObject>>, String> {
    ...
    for artboard_spec in &artboard_specs {
        ...
        if let Some(origin_x) = artboard_spec.origin_x {
            artboard.origin_x = origin_x;
        }
        if let Some(origin_y) = artboard_spec.origin_y {
            artboard.origin_y = origin_y;
        }
        if let Some(x) = artboard_spec.x {
            artboard.x = x;
        }
        if let Some(y) = artboard_spec.y {
            artboard.y = y;
        }
        if artboard_spec
            .state_machines
