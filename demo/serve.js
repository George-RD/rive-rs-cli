...
const FIXTURE_OVERRIDES = {
  ...
  official_test: {
    ...
    scope: 'visual',
    tags: ['interactive', 'animated', 'official'],
...
function inferFixtureMetadata(fixtureName, spec) {
  ...
  const metadata = {
    ...
    animationsByArtboard: hasAnimations ? Object.fromEntries(
      artboards
        .filter((artboard) => typeof artboard?.name === 'string')
        .map((artboard) => [
          artboard.name,
          (artboard.animations || []).map((a) => a.name).filter(Boolean)
        ])
    ) : undefined,
