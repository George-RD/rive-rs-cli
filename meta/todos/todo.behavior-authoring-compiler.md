...
PR #224 / issue #223 validates the fully qualified identities of the emitted typed-behavior source entries before the shared compiler appends them or mutates the scene with behavior output. A repeated identity reports `behavior_source_id_collision` at the later entry's authored `.id` path, naming the identity and its first claimant. Entries are checked in the lowerer's existing deterministic authored-path order; the first claimant is retained when more than two kinds collide. The existing same-kind, input/binding, and region-id checks still run first and keep their diagnostics. Local IDs remain reusable across statecharts and parallel regions when their fully qualified identities differ. This is a bounded typed-behavior check, not a global namespace rule across visual, motion, raw escapes, and behavior. It introduces no schema fields, runtime types, output rewriting, or additional lowering pass.
...
Thirteen public contracts in `tests/authoring_trigger_binding_contract.rs` cover
native encoded output, the strict property and condition forms, kind-agreement and
unknown-reference diagnostics, blend-source rejection, region-only discovery, shared
bindings, unbound byte compatibility, deterministic lowering and source-map identity,
canonical property-kind and resolution failures, and atomic rollback.
...
samples: the model trigger transitions, the synthesized input alone does not, the
fired trigger is momentary rather than latching, a declared machine trigger returns
the pair, and the model trigger re-engages.
...
missing property name before the object fix and passed after it. Final exact-head
CI/MSRV and separate Standards/Spec self-review are recorded on the pull request
for #251.
