...
The typed continuation on branch `claude/workflows-roadmap-demos-9wlqz4` adds number and trigger inputs, comparison and trigger conditions, one-dimensional blend states, parallel regions, and typed listener actions to the same compiler path. It is implemented and not merged; no CI run is recorded for it here.
