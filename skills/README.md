...
console adds explicit `stacking`, motion `continuity`, a 1D blend state driven by
a number input, and parallel statechart regions. They do not imply that every
Rive concept is typed; read the current field names and enum values from
