...
fn numeric_view_model_binding_compiles_to_a_model_condition_not_an_input_condition() {
    ...
    let parsed =
        parse_riv(&compile(&lowered.scene), &InspectFilter::default()).expect("encoded scene");
...
fn canonical_number_binding_rejects_non_numeric_conditions() {
    ...
    for value in [json!(true), Value::Null] {
        ...
        invalid["artboard"]["state_machines"][0]["layers"][0]["transitions"][1]["conditions"][0]
            ["value"] = value;
        let invalid: SceneSpec = serde_json::from_value(invalid).expect("canonical scene");
        let error =
            compile_scene(&invalid, None, 0).expect_err("bound number condition must be numeric");
        assert!(error.to_string().contains("number"), "{error}");
    }
}
...
fn numeric_model_property_uses_the_runtime_view_model_name_field() {
    ...
    let scene = lower(&document()).scene;
    let expected_name = scene["artboard"]["children"][1]["children"][0]["name"]
        .as_str()
        .expect("name");
    ...
    let property = parsed
        .objects
        .iter()
        .find(|object| object.type_key == type_keys::VIEW_MODEL_PROPERTY_NUMBER)
        .expect("model property");
    assert!(property.properties.iter().any(|field| field.key
        == property_keys::VIEW_MODEL_COMPONENT_NAME
        && field.value == PropertyValueRead::String(expected_name.to_string())));
    assert!(!property.properties.iter().any(|field| matches!(
        field.key,
        property_keys::COMPONENT_NAME | property_keys::COMPONENT_PARENT_ID
    )));
}
...
fn every_numeric_binding_comparison_keeps_its_encoded_operator_and_value() {
    ...
    for (comparison, operator, encoded) in [
        ...
        let mut input = document();
        input["behavior"]["statecharts"][0]["transitions"][0]["when"]["compare"] =
            json!(comparison);
        ...
        let parsed =
            parse_riv(&compile(&first.scene), &InspectFilter::default()).expect("encoded scene");
        let condition = parsed
            .objects
            .iter()
            .find(|object| object.type_key == type_keys::TRANSITION_VIEW_MODEL_CONDITION)
            .expect("condition");
        let actual = condition
            .properties
            .iter()
            .find(|field| field.key == property_keys::TRANSITION_VIEW_MODEL_CONDITION_OP_VALUE)
            .map(|field| field.value.clone())
            .unwrap_or(PropertyValueRead::UInt(0));
        assert_eq!(actual, PropertyValueRead::UInt(encoded), "{comparison}");
        let comparator = parsed
            .objects
            .iter()
            .find(|object| object.type_key == type_keys::TRANSITION_VALUE_NUMBER_COMPARATOR)
            .expect("comparator");
        assert!(comparator.properties.iter().any(|field| field.key
            == property_keys::TRANSITION_VALUE_NUMBER_COMPARATOR_VALUE
            && field.value == PropertyValueRead::Float(60.0)));
    }
}
...
fn model_defaults_and_thresholds_use_document_parameters() {
    let input: Value =
        serde_json::from_str(include_str!("../examples/authoring/number-binding.v0.json"))
            .expect("parameterized example");
    let output = lower(&input);
    let machine = &output.scene["artboard"]["state_machines"][0];
    assert_eq!(machine["inputs"][0]["value"], 25.0);
    assert_eq!(
        machine["layers"][0]["transitions"][1]["conditions"][0]["value"],
        60.0
    );
    assert_eq!(
        machine["layers"][0]["transitions"][2]["conditions"][0]["value"],
        60.0
    );
...
fn numeric_expression_errors_keep_model_and_condition_authored_paths() {
    for (pointer, path) in [
        (
            "/behavior/models/0/properties/0/value",
            "$.behavior.models[0].properties[0].value",
        ),
        (
            "/behavior/statecharts/0/transitions/0/when/value",
            "$.behavior.statecharts[0].transitions[0].when.value",
        ),
    ] {
        for (expression, code, suffix) in [
            ...
            (
                json!({"kind": "literal", "value": 5.0, "unit": "px"}),
                "unit_mismatch",
                "",
            ),
            (
                json!({"kind": "literal", "value": f64::MAX, "unit": "scalar"}),
                "numeric_out_of_range",
                ".value",
            ),
            (
                json!({"kind": "literal", "value": 1e-100, "unit": "scalar"}),
                "numeric_out_of_range",
                ".value",
            ),
        ] {
    ...
    let mut input = document();
    input["behavior"]["models"][0]["properties"][0]["value"] =
        json!({"kind": "parameter", "name": "missing"});
    ...
    assert_diagnostic(
        &input,
        "unknown_parameter",
        "$.behavior.models[0].properties[0].value.name",
    );
}
...
fn numeric_binding_reference_errors_are_authored_and_specific() {
    ...
    for field in ["view_model", "property"] {
        let mut scene = lower(&document()).scene;
        scene["artboard"]["state_machines"][0]["inputs"][0]["view_model_binding"][field] =
            json!("missing");
        let scene: SceneSpec = serde_json::from_value(scene).expect("canonical scene");
...
fn numeric_bindings_are_shared_within_a_chart_and_scoped_across_charts() {
    ...
    assert_ne!(
        machines[0]["inputs"][0]["name"],
        machines[1]["inputs"][0]["name"]
    );
    ...
    compile(&output.scene);
    input["behavior"]["statecharts"][0]["regions"][0]["transitions"][0]["when"] =
        json!({"binding": "gate-enabled", "equals": true});
    assert_diagnostic(
        &input,
        "invalid_condition_binding",
        "$.behavior.statecharts[0].regions[0].transitions[0].when.binding",
    );
}
...
fn numeric_binding_ids_count_models_and_properties_not_visual_objects() {
    ...
    let paths: Vec<_> = objects
        ...
        .filter(|property| property.key == property_keys::DATA_BIND_CONTEXT_SOURCE_PATH_IDS)
...
fn unbound_number_and_boolean_conditions_keep_their_existing_object_families() {
    ...
    unbound["artboard"]["state_machines"][0]["inputs"][0]
        .as_object_mut()
        .expect("input")
        .remove("view_model_binding");
    let parsed = parse_riv(&compile(&unbound), &InspectFilter::default()).expect("number input");
    assert!(
        parsed
            .objects
            .iter()
            .any(|object| object.type_key == type_keys::TRANSITION_NUMBER_CONDITION)
    );
    assert!(
        !parsed
            .objects
            .iter()
            .any(|object| object.type_key == type_keys::TRANSITION_VIEW_MODEL_CONDITION)
    );
    let boolean: Value = serde_json::from_str(include_str!(
        "../examples/authoring/behavior-binding.v0.json"
    ))
    .expect("boolean example");
    let parsed = parse_riv(&compile(&lower(&boolean).scene), &InspectFilter::default())
        .expect("boolean binding");
    assert!(
        parsed
            .objects
            .iter()
            .any(|object| object.type_key == type_keys::BINDABLE_PROPERTY_BOOLEAN)
    );
    assert!(
        parsed
            .objects
            .iter()
            .any(|object| object.type_key == type_keys::TRANSITION_VALUE_BOOLEAN_COMPARATOR)
    );
    assert!(
        !parsed
            .objects
            .iter()
            .any(|object| object.type_key == type_keys::BINDABLE_PROPERTY_NUMBER)
    );
}
...
fn invalid_numeric_binding_replacement_is_atomic() {
    ...
    let error =
        apply_operations(&spec, &operations).expect_err("mismatched binding must roll back");
    assert!(
        error
            .diagnostics
            .iter()
            .any(|diagnostic| diagnostic.code == "invalid_condition_binding")
    );
