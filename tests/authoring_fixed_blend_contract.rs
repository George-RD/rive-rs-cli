...
fn set_weight(input: &mut Value, weight: Value) {
    input["behavior"]["statecharts"][0]["states"][0]["direct_blend"]["motions"][1]["weight"] =
        weight;
}

fn compiled(scene: &Value) -> Vec<u8> {
    compile_scene(
        &serde_json::from_value(scene.clone()).expect("canonical scene"),
        None,
        0,
    )
...
fn fixed_weights_lower_to_native_constants_without_synthetic_inputs() {
    ...
    let parsed =
        parse_riv(&compiled(&first.scene), &InspectFilter::default()).expect("encoded fixed blend");
...
fn fixed_percentages_accept_boundaries_fractions_and_parameter_arithmetic() {
    for value in [0.0, -0.0, 0.25, 25.5, 99.999999, 100.0] {
        ...
        assert_eq!(
            lowered.scene["artboard"]["state_machines"][0]["layers"][0]["states"][1]["children"][1]
                ["mix_value"],
            json!(value)
        );
        let parsed =
            parse_riv(&compiled(&lowered.scene), &InspectFilter::default()).expect("valid binary");
        let direct = parsed
            .objects
            .iter()
            .filter(|object| object.type_key == type_keys::BLEND_ANIMATION_DIRECT)
            .nth(1)
            .expect("weighted motion");
        let encoded = direct
            .properties
            .iter()
            .find(|field| field.key == property_keys::BLEND_ANIMATION_DIRECT_MIX_VALUE)
            .map(|field| field.value.clone())
            .unwrap_or(PropertyValueRead::Float(100.0));
...
fn fixed_sources_are_exclusive_and_the_published_schema_keeps_each_form_strict() {
    ...
    let forms = schema["$defs"]["BehaviorDirectBlendMotionSpec"]["anyOf"]
        .as_array()
        .expect("exclusive schema forms");
...
fn invalid_fixed_expressions_retain_precise_authored_diagnostics() {
    for (expression, code, suffix) in [
        (
            json!({"kind":"literal", "value":25, "unit":"px"}),
            "unit_mismatch",
            "",
        ),
        (
        ...
        (
            json!({"kind":"divide", "value":literal(25.0), "divisor":0}),
            "division_by_zero",
            ".divisor",
        ),
    ...
    let mut input = document();
    input["behavior"]["statecharts"][0]["states"][0]["direct_blend"]["motions"][1]["motion"] =
        json!("missing");
    assert_diagnostic(
        &input,
...
fn programmatic_non_finite_parameters_cannot_reach_fixed_weight_encoding() {
    ...
    set_weight(
        &mut input,
        json!({"kind":"parameter", "name":"contribution"}),
    );
    for value in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        ...
        assert!(
            error
                .diagnostics
                .iter()
                .any(|entry| entry.code == "non_finite"),
            "{error:?}"
        );
    }
}
...
fn fixed_weights_in_regions_share_parameters_and_preserve_source_identity() {
    ...
    region["states"][0]["direct_blend"]["motions"][1]["weight"] =
        json!({"kind":"parameter", "name":"contribution"});
    ...
    assert_eq!(
        first.scene["artboard"]["state_machines"][0]["layers"][1]["states"][1]["children"][1]["mix_value"],
        62.5
    );
    let source = first
        .source_map
        .entries
        .iter()
        .find(|entry| entry.authored_id == "meter/overlay/reading")
        .expect("region state source");
    assert_eq!(
        source.scene_paths,
    ...
    for entry in &first.source_map.entries {
        for path in &entry.scene_paths {
            assert!(first.scene.pointer(path).is_some(), "{path}");
        }
    }
    compiled(&first.scene);
    input["behavior"]["statecharts"][0]["regions"][0]["states"][0]["direct_blend"]["motions"][1]
        ["weight"] = literal(100.000001);
    assert_diagnostic(
        &input,
        "invalid_blend_weight",
        "$.behavior.statecharts[0].regions[0].states[0].direct_blend.motions[1].weight",
    );
}
...
fn fixed_input_and_model_sources_keep_real_indices_across_charts() {
    let mut input: Value = serde_json::from_str(include_str!(
        "../examples/authoring/model-blend-panel.v0.json"
    ))
    .expect("model panel");
    let chart = &mut input["behavior"]["statecharts"][0];
    chart["inputs"][0]["id"] = json!("left-control");
    chart["states"][0]["direct_blend"]["motions"][0] =
        json!({"motion":"rest-track", "weight":literal(100.0)});
    chart["states"][0]["direct_blend"]["motions"][1] =
        json!({"motion":"left-track", "input":"left-control"});
    let mut earlier = chart.clone();
    earlier["id"] = json!("earlier");
    earlier["states"][0]["direct_blend"]["motions"][2] =
        json!({"motion":"right-track", "weight":literal(25.0)});
    ...
    assert_eq!(
        machines[1]["layers"][0]["states"][1]["children"][2]["input_id"],
        0
    );
    let binding = output
        .source_map
        .entries
        .iter()
        .find(|entry| entry.authored_id == "right-model")
        .expect("used model binding");
    ...
    assert!(
        !output
            ...
            .entries
            .iter()
            .any(|entry| entry.authored_id == "left-model")
    );
    let parsed =
        parse_riv(&compiled(&output.scene), &InspectFilter::default()).expect("mixed binary");
    assert_eq!(
        parsed
            .objects
            .iter()
...
fn fixed_child_order_remains_authored_and_does_not_rewrite_source_identity() {
    ...
    expected["artboard"]["state_machines"][0]["layers"][0]["states"][1]["children"]
        .as_array_mut()
        .expect("children")
        .reverse();
...
fn invalid_fixed_weight_edit_rolls_back_atomically() {
    ...
    assert!(
        error
            .diagnostics
            .iter()
            .any(|entry| entry.code == "invalid_blend_weight")
    );
    assert_eq!(
        snapshot,
...
fn legacy_model_direct_and_one_dimensional_binaries_are_unchanged() {
    ...
    for (input, expected) in [
        (
            include_str!("../examples/authoring/model-blend-panel.v0.json"),
            "bc107cad1542c95190a125b24ce97a6c62e05fd553972bc47f2cad3735c5f8d4",
        ),
        (
            include_str!("../examples/authoring/model-blend-1d-panel.v0.json"),
            "bdb14f4f4a5492467d7ff04050f973f652337c9bab59dec6be909d49c3f19d94",
        ),
    ] {
        let lowered = lower_authoring_json(input).expect("legacy example");
        assert_eq!(
            format!("{:x}", Sha256::digest(compiled(&lowered.scene))),
            expected
        );
    }
}
