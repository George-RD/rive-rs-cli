...
fn explicit_always_guard_compiles_a_timed_transition_without_inputs() {
    ...
    let parsed =
        parse_riv(&binary, &InspectFilter::default()).expect("encoded automatic transition");
    for key in [
...
fn automatic_guard_round_trips_through_the_typed_api_without_source_map_drift() {
    ...
    assert_eq!(
        serialized["behavior"]["statecharts"][0]["transitions"][0]["when"],
        "always"
    );
    ...
    assert_eq!(
        source.scene_paths,
        ["/artboard/state_machines/0/layers/0/transitions/1"]
    );
}
...
fn automatic_guards_are_explicit_and_cannot_hide_in_leaf_groups() {
    ...
    let mut input = document();
    input["behavior"]["statecharts"][0]["transitions"][0]
        .as_object_mut()
        .expect("transition")
        .remove("when");
    assert_diagnostic(&input, "invalid_json", "$");
    ...
    assert_eq!(
        schema["$defs"]["BehaviorAlwaysGuardSpec"]["enum"],
        json!(["always"])
    );
    assert!(
        schema["$defs"]["BehaviorTransitionSpec"]["required"]
            .as_array()
...
fn root_and_region_automatic_sequences_keep_independent_named_identity() {
    ...
    .expect("automatic example");
    let lowered = lower(&input);
    let machine = &lowered.scene["artboard"]["state_machines"][0];
    assert_eq!(machine["inputs"], json!([]));
    for (layer, gate, id) in [
        (0, 500, "sequence/advance"),
        (1, 1000, "sequence/lower/advance"),
    ] {
        assert_eq!(
            machine["layers"][layer]["transitions"][1],
        ...
        let source = lowered
            ...
            .entries
            .iter()
            .find(|entry| entry.authored_id == id)
            .expect("scoped transition");
        assert_eq!(
            source.scene_paths,
            [format!(
                "/artboard/state_machines/0/layers/{layer}/transitions/1"
            )]
        );
    }
...
fn automatic_transitions_compose_with_bound_and_all_guards_without_extra_inputs() {
    ...
    assert_eq!(
        machine["inputs"],
        baseline.scene["artboard"]["state_machines"][0]["inputs"]
    );
    assert_eq!(
        machine["layers"][0]["transitions"][1]["conditions"],
        json!([])
    );
    assert_eq!(
    ...
    assert_eq!(
        machine["layers"][0]["transitions"][3]["conditions"][0]["input"],
        machine["inputs"][0]["name"]
    );
...
fn automatic_timing_keeps_omission_zero_expressions_and_authored_errors() {
    ...
    assert!(
        omitted.scene["artboard"]["state_machines"][0]["layers"][0]["transitions"][1]
            .get("exit_time")
            .is_none()
    );
    ...
    assert_eq!(
        timed.scene["artboard"]["state_machines"][0]["layers"][0]["transitions"][1],
    ...
    assert_diagnostic(
        &input,
        "unknown_parameter",
        "$.behavior.statecharts[0].transitions[0].exit_time_ms.name",
    );
}
...
fn automatic_statechart_edits_commit_atomically_and_bad_references_roll_back() {
    ...
    for valid in [true, false] {
        ...
        if !valid {
            replacement["transitions"][0]["to"] = json!("missing");
        }
        let operations = [
        ...
        if valid {
            ...
            assert_eq!(
                applied.lowered.scene["artboard"]["state_machines"][0]["layers"][0]["transitions"]
                    [1]["duration"],
                json!(250)
            );
            assert_eq!(applied.lowered.source_map, before.source_map);
            assert_eq!(
                lower_authoring(&applied.spec).expect("edited document"),
                applied.lowered
            );
        } else {
            assert!(result.is_err(), "bad target must roll back");
        }
