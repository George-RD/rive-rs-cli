...
use rive_cli::compile::{EmbeddedCompileError, CompileOptions, compile_scene, compile_scene_with_assets};
...
fn options() -> CompileOptions {
    CompileOptions { file_id: FILE_ID, asset_limits: AssetLimits::default() }
}
...
fn public_memory_compilation_embeds_image_and_font_without_a_base_directory() {
    ...
    let lengths = parsed.objects.iter()
...
fn empty_wrong_kind_and_budget_failures_retain_logical_asset_identity() {
    for (bytes, code) in [(&[][..], "asset-empty"), (IMAGE, "asset-kind-mismatch")] {
        ...
        let error = compile_scene_with_assets(&spec(), options(), &assets).expect_err("invalid font");
        assert_eq!(error.code(), code);
        assert!(matches!(error, EmbeddedCompileError::Asset(ref error) if error.asset_name == "InterBold"));
    }
    let error = compile_scene_with_assets(&spec(), CompileOptions {
        file_id: FILE_ID,
        asset_limits: AssetLimits { per_asset_bytes: FONT.len() - 1, total_bytes: usize::MAX },
    }, &assets()).expect_err("per-asset budget");
    assert_eq!(error.code(), "asset-too-large");
    let error = compile_scene_with_assets(&spec(), CompileOptions {
        file_id: FILE_ID,
        asset_limits: AssetLimits { per_asset_bytes: usize::MAX, total_bytes: FONT.len() + IMAGE.len() - 1 },
    }, &assets()).expect_err("total budget");
    assert_eq!(error.code(), "asset-total-budget-exceeded");
    assert!(matches!(error, EmbeddedCompileError::Asset(ref error) if error.asset_name == "Aurora"));
}
...
fn structural_validation_precedes_caller_asset_resolution() {
    ...
    let error = compile_scene_with_assets(&invalid, options(), &NoReads).expect_err("duplicate name");
...
fn an_explicit_external_asset_declaration_does_not_request_missing_embedded_bytes() {
    let mut document = document();
    for index in [0, 1] {
        document["artboard"]["children"][index].as_object_mut().expect("asset").remove("source");
    }
    let scene = serde_json::from_value(document).expect("external asset scene");
    let bytes = compile_scene_with_assets(&scene, options(), &NoReads).expect("no embedded sources");
    let parsed = parse_riv(&bytes, &InspectFilter::default()).expect("external asset binary");
    assert!(!parsed.objects.iter().any(|object| object.type_key == type_keys::FILE_ASSET_CONTENTS));
}
...
fn successful_legacy_scene_and_authoring_fixture_digests_remain_unchanged() {
    let baseline: Value = serde_json::from_str(include_str!("evidence/embedded-assets/legacy-digests.json"))
    ...
    assert!(digests.keys().any(|path| path.starts_with("examples/authoring/")));
    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    for (relative, expected) in digests {
        ...
        let scene: SceneSpec = if document.get("authoring_format_version").is_some() {
            let lowered = rive_cli::authoring::lower_authoring_json(&input).expect("authoring lowering");
            serde_json::from_value(lowered.scene).expect("lowered scene")
        } else {
            serde_json::from_value(document).expect("scene")
        };
        let bytes = compile_scene(&scene, path.parent(), file_id).expect("legacy successful fixture");
        assert_eq!(format!("{:x}", Sha256::digest(bytes)), expected.as_str().expect("digest"), "{relative}");
    }
}
...
fn total_asset_budget_is_file_scoped_across_artboards() {
    ...
    let error = compile_scene_with_assets(&scene, CompileOptions {
        file_id: FILE_ID,
        asset_limits: AssetLimits { per_asset_bytes: FONT.len(), total_bytes: FONT.len() * 2 - 1 },
    }, &assets()).expect_err("two emitted payloads exceed the file budget");
    assert_eq!(error.code(), "asset-total-budget-exceeded");
    assert!(matches!(error, EmbeddedCompileError::Asset(ref error) if error.asset_name == "SecondFont"));
}
...
fn duplicate_file_scoped_asset_names_fail_before_any_resolver_call() {
    ...
    let error = compile_scene_with_assets(&scene, options(), &NoReads).expect_err("file-scope duplicate");
    assert_eq!(error.code(), "invalid-scene");
}
