...
fn every_kind() -> Value {
    ...
    input["behavior"]["models"][0]["properties"].as_array_mut().expect("properties").push(
        json!({ "kind": "number", "id": "level", "value": scalar(25.0) }),
    );
    input["behavior"]["bindings"].as_array_mut().expect("bindings").push(
        json!({ "id": "gate-level", "model": "gate-model", "property": "level" }),
    );
    input["behavior"]["statecharts"][0]["inputs"].as_array_mut().expect("inputs").push(
        json!({ "kind": "trigger", "id": "release" }),
    );
    let all = input.pointer_mut(WHEN).expect("guard")["all"].as_array_mut().expect("conditions");
...
fn all_guard_lowers_mixed_named_conditions_in_authored_order() {
    ...
    assert_eq!(lowered, lower_authoring_json(&input.to_string()).expect("repeat lowering"));
...
fn each_leaf_keeps_its_single_condition_bytes_and_source_map() {
    ...
    for leaf in template.pointer(WHEN).expect("guard")["all"].as_array().expect("leaves") {
...
fn every_condition_kind_retains_its_native_encoded_type() {
    ...
    let count = |key| parsed.objects.iter().filter(|object| object.type_key == key).count();
...
fn region_only_bindings_are_collected_and_shared_with_blend_consumers() {
    for with_blend in [false, true] {
        ...
        if with_blend {
            assert_eq!(machine["layers"][0]["states"][1]["input"], conditions[3]["input"]);
        }
        let source = lowered.source_map.entries.iter().find(|entry| entry.authored_id == "gate/secondary/engage").expect("region transition source");
...
fn empty_and_oversized_groups_fail_on_json_and_typed_paths_in_each_scope() {
    for in_region in [false, true] {
        for count in [0, 1001] {
            ...
            let from_json = lower_authoring_json(&input.to_string()).expect_err("invalid JSON count");
...
fn a_thousand_conditions_are_supported_without_changing_the_group_limit() {
    ...
    *input.pointer_mut(WHEN).expect("guard") = json!({
    ...
    assert_eq!(lowered.scene["artboard"]["state_machines"][0]["layers"][0]["transitions"][1]["conditions"].as_array().expect("conditions").len(), 1000);
    assert!(!compile(&lowered.scene).is_empty());
    let schema = authoring_schema();
    let group = schema["$defs"]["BehaviorTransitionGuardSpec"]["anyOf"].as_array().expect("guard variants").iter().find(|variant| variant["properties"]["all"].is_object()).expect("all variant");
...
fn leaf_reference_errors_report_the_indexed_authored_path() {
    for (leaf, code, field) in [
        (json!({ "input": "missing", "equals": true }), "unknown_behavior_input", "input"),
        (json!({ "input": "load", "equals": true }), "invalid_condition_input", "input"),
        (json!({ "binding": "missing", "equals": true }), "unknown_behavior_binding", "binding"),
        (json!({ "binding": "gate-enabled", "compare": "equal", "value": scalar(0.0) }), "invalid_condition_binding", "binding"),
        (json!({ "trigger": "armed" }), "invalid_condition_input", "trigger"),
    ] {
        let mut input = document();
        input.pointer_mut(WHEN).expect("guard")["all"][1] = leaf;
        assert_diagnostic(&input, code, &format!("$.behavior.statecharts[0].transitions[0].when.all[1].{field}"));
    }
}
...
fn numeric_guard_expressions_keep_parameters_timing_and_error_paths() {
    ...
    input.pointer_mut(WHEN).expect("guard")["all"][2]["value"] = json!({ "kind": "parameter", "name": "threshold" });
    ...
    assert!(!compile(&lowered.scene).is_empty());
    input.pointer_mut(WHEN).expect("guard")["all"][2]["value"] = json!({ "kind": "parameter", "name": "missing" });
    assert_diagnostic(&input, "unknown_parameter", "$.behavior.statecharts[0].transitions[0].when.all[2].value.name");
}
...
fn a_bad_guard_rolls_back_a_statechart_operation_batch() {
    ...
    replacement["transitions"][0]["when"]["all"][1] = json!({ "binding": "missing", "equals": true });
    let operations = [
        AuthoringOperation::Remove {
            target: AuthoringTarget::BehaviorStatechart { target_id: "gate".to_string() },
        },
        AuthoringOperation::Insert {
            entity: AuthoringEntity::BehaviorStatechart(serde_json::from_value(replacement).expect("replacement chart")),
    ...
    assert!(error.diagnostics.iter().any(|diagnostic| diagnostic.code == "unknown_behavior_binding" && diagnostic.path == "$.behavior.statecharts[0].transitions[0].when.all[1].binding"));
    assert_eq!(serde_json::to_value(&spec).expect("unchanged document"), snapshot);
